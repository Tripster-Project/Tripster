...
// Arrays containing the hotels/etc generated by the algorithm (not ones added by the user)
var hotelResultsArr = [];
var foodResultsArr = [];
var gasResultsArr = [];

// Arrays containing the hotels/etc added by the user manually
var userHotels = [];
var userFood = [];
var userGas = [];
var check_radius = 100000;
...
var add_hotels_button = document.getElementById('add-hotels-button');
var add_food_button = document.getElementById('add-food-button');
var add_gas_button = document.getElementById('add-gas-button');

add_hotels_button.addEventListener("click", add_user_hotels);
add_food_button.addEventListener("click", add_user_food);
add_gas_button.addEventListener("click", add_user_gas);
...
function init_autocomplete_inputs(){
    var start_input = document.getElementById('start');
		...
		var add_hotels_input = document.getElementById('add-hotels-input');
    var add_food_input = document.getElementById('add-food-input');
    var add_gas_input = document.getElementById('add-gas-input');

    var add_hotels_autocomplete = new google.maps.places.Autocomplete(add_hotels_input);
    var add_food_autocomplete = new google.maps.places.Autocomplete(add_food_input);
    var add_gas_autocomplete = new google.maps.places.Autocomplete(add_gas_input);
...
function hotels_callback(results, status, currLoc){
    ...
    if (status == google.maps.places.PlacesServiceStatus.OK) {
    		var use_generated_hotels = true;
        var userHotelsIndexes = [];
				var result = sort_by_distance(currLoc, results);

        for(var i = 0; i < userHotels.length; i++){
        		if(get_distance(result.geometry.location, userHotels[i].geometry.location) <= check_radius){
            		use_generated_hotels = false;
                userHotelsIndexes.push(i);
            }
        }

        if(use_generated_hotels){
        		add_hotel_marker(result);
    				hotelResultsArr.push(result);
        }else{
        		for(var i = 0; i < userHotelsIndexes.length; i++){
            		add_hotel_marker(userHotels[userHotelsIndexes[i]]);
    						hotelResultsArr.push(userHotels[userHotelsIndexes[i]]);
            }
        }
...
function food_callback(results, status, currLoc){
    ...
    // Otherwise we would get wayyy too many results to have to sort through
    if (status == google.maps.places.PlacesServiceStatus.OK) {
    		var use_generated_food = true;
        var userFoodIndexes = [];
				var result = sort_by_distance(currLoc, results);

        for(var i = 0; i < userFood.length; i++){
        		if(get_distance(result.geometry.location, userFood[i].geometry.location) <= check_radius){
            		use_generated_food = false;
                userFoodIndexes.push(i);
            }
        }

        if(use_generated_food){
        		add_food_marker(result);
    				foodResultsArr.push(result);
        }else{
        		for(var i = 0; i < userFoodIndexes.length; i++){
            		add_food_marker(userFood[userFoodIndexes[i]]);
    						foodResultsArr.push(userFood[userFoodIndexes[i]]);
            }
        }
...
function gas_callback(results, status, currLoc){
		if (status == google.maps.places.PlacesServiceStatus.OK) {
    		var use_generated_gas = true;
        var userGasIndexes = [];
				var result = sort_by_distance(currLoc, results);

        for(var i = 0; i < userGas.length; i++){
        		if(get_distance(result.geometry.location, userGas[i].geometry.location) <= check_radius){
            		use_generated_gas = false;
                userGasIndexes.push(i);
            }
        }

        if(use_generated_gas){
        		add_gas_marker(result);
    				gasResultsArr.push(result);
        }else{
        		for(var i = 0; i < userGasIndexes.length; i++){
            		add_gas_marker(userGas[userGasIndexes[i]]);
    						gasResultsArr.push(userGas[userGasIndexes[i]]);
            }
        }
...
function add_user_hotels(){
		var input_val = document.getElementById('add-hotels-input').value;
    if(input_val == ''){
    		alert('Please enter a hotel to add');
    }else{
    		var request = {
        		query: input_val,
    		...
    		var service = new google.maps.places.PlacesService(map);
        service.findPlaceFromQuery(request, add_user_hotels_callback);
    }
}

function add_user_food(){
		var input_val = document.getElementById('add-food-input').value;
    if(input_val == ''){
    		alert('Please enter a food to add');
    }else{
    		var request = {
        		query: input_val,
            fields: ['name', 'geometry']
    		...
    		var service = new google.maps.places.PlacesService(map);
        service.findPlaceFromQuery(request, add_user_food_callback);
    }
}

function add_user_gas(){
		var input_val = document.getElementById('add-gas-input').value;
    if(input_val == ''){
    		alert('Please enter a gas station to add');
    }else{
    		var request = {
        		query: input_val,
            fields: ['name', 'geometry']
    		...
    		var service = new google.maps.places.PlacesService(map);
        service.findPlaceFromQuery(request, add_user_gas_callback);
    }
}

// ** NOTE FROM GREG **: To add hotels/gas/food items to the route results do it from these functions
function add_user_hotels_callback(results, status){
		if (status == google.maps.places.PlacesServiceStatus.OK) {
				var input_box = document.getElementById('add-hotels-input');
        input_box.value = '';
        userHotels.push(results[0]);
    }else{
    		console.log('Error: ');
        console.log(status);
    }
...
function add_user_food_callback(results, status){
		if (status == google.maps.places.PlacesServiceStatus.OK) {
				var input_box = document.getElementById('add-food-input');
        input_box.value = '';
        userFood.push(results[0]);
    }else{
    		console.log('Error: ');
        console.log(status);
    }
...
function add_user_gas_callback(results, status){
		if (status == google.maps.places.PlacesServiceStatus.OK) {
				var input_box = document.getElementById('add-gas-input');
        input_box.value = '';
        userGas.push(results[0]);
    }else{
    		console.log('Error: ');
        console.log(status);
    }
